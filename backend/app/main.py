...
class AskRequest(BaseModel):
    ...
    # Phase 5: real identity for role-scoped MCP tool access. Absent for any
    # caller not yet updated to send them — run_agent() falls back to the
    # exact pre-Phase-5 behavior (native tools only) when either is missing.
...
async def ask_question(req: AskRequest):
    try:
        ...
        # run_agent is natively async (Phase 5: it awaits MultiServerMCPClient
        # tool loading internally) — awaited directly now, not via
        # asyncio.to_thread (that was only ever needed to keep the old fully-
        # synchronous implementation off the event loop).
        answer_text = await run_agent(
            formatted_input, history, system_prompt,
            role=req.role, farmer_id=req.farmerId,
        )
        if not answer_text:
...
async def get_risk_brief(req: RiskBriefRequest):
    ...
    try:
        ...
        brief_text = await run_agent(internal_prompt)
