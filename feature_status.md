...
Disease & pest vision scanner — 🔧 Phase 1 (on-device scanner, coverage gate, exact CAM overlay, KVK escalation) + Phase 2 (scan fused into the AI assistant, pesticide-dosage safety rule) code-complete on `main`; see COMPUTER-VISION.md. Pending: `farm_scans` migration applied (Supabase access), threshold calibration, cited disease corpus, and the trained hybrid CNN+ViT — Phase 1 ships a pretrained MobileNetV2 on PlantVillage alone, covering 6 crops (corn, grape, potato, pepper, strawberry, tomato)
